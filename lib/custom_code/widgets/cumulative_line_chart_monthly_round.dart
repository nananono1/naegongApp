...
class CumulativeLineChartMonthlyRound extends StatefulWidget {
  const CumulativeLineChartMonthlyRound({
    super.key,
    this.width,
    this.height,
  ...
  @override
  State<CumulativeLineChartMonthlyRound> createState() =>
      _CumulativeLineChartMonthlyRoundState();
}

class _CumulativeLineChartMonthlyRoundState
    extends State<CumulativeLineChartMonthlyRound> {
  @override
  Widget build(BuildContext context) {
    // 기준일로부터 30일간의 데이터를 필터링
    ...
    return Container(
      ...
      child: SfCartesianChart(
        ...
        title: ChartTitle(text: 'Cumulative Study Hours (Last 30 Days)'),
        ...
        tooltipBehavior: TooltipBehavior(
            enable: true,
            tooltipPosition:
                TooltipPosition.pointer), // 툴팁을 포인터 기준으로 아래쪽에 표시, // 툴팁 사용 설정
  ...
  List<SplineSeries<Map<String, dynamic>, String>> _buildSplineSeries(
    ...
    return cumulativeData.entries.map((entry) {
      return SplineSeries<Map<String, dynamic>, String>(
        ...
        markerSettings: MarkerSettings(isVisible: false), // 마커 표시
        dataLabelSettings: DataLabelSettings(isVisible: true), // 데이터 라벨 표시
        dataLabelMapper: (data, _) =>
            data['cumulativeHours'].toStringAsFixed(1), // 소숫점 첫번째 자리까지만 표시
      );
    }).toList();
  }
  ...
  Map<String, List<Map<String, dynamic>>> _calculateCumulativeStudyHours(
    ...
    // 기준일로부터 30일간의 날짜 리스트 생성 (MM/dd 형식으로 변경)
